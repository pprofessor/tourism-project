...
public class AuthController {
    ...
    private String generateSimpleOTP() {
        ...
        // ایجاد یک بخش با 3 رقم تکراری
        int repeatedDigit = random.nextInt(10);
        String repeatedPart = String.valueOf(repeatedDigit).repeat(3);

        // ایجاد اعداد رندوم برای قسمت‌های دیگر (مجموعاً 3 رقم دیگر)
        int randomPart = random.nextInt(1000); // عدد 0-999

        // روش 1: سه رقم رندوم + سه رقم تکراری
        if (random.nextBoolean()) {
            return String.format("%03d%s", randomPart, repeatedPart);
        }
        // روش 2: سه رقم تکراری + سه رقم رندوم
        else {
            return String.format("%s%03d", repeatedPart, randomPart);
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> sendVerificationCode(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            logger.info("OTP code generated: {} for mobile: {}", verificationCode, mobile);
            // Search for user
            Optional<User> userOpt = userRepository.findByMobile(mobile);
    ...
    public ResponseEntity<Map<String, Object>> verifyCode(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            logger.info("       Verifying code For Mobile: {}, Code: {}", standardizedMobile, code);
            ...
            logger.error("Error verifying OTP code for mobile {}: {}", mobile, e.getMessage(), e);
            response.put("success", false);
    ...
    public ResponseEntity<Map<String, Object>> forgotPassword(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            if (email == null || email.trim().isEmpty()) {
                response.put("success", false);
                response.put("message", "ایمیل الزامی است");
                return ResponseEntity.badRequest().body(response);
            }
            ...
            if (!userOpt.isPresent()) {
                response.put("success", false);
                response.put("message", "کاربری با این ایمیل یافت نشد");
                return ResponseEntity.badRequest().body(response);
            }

            User user = userOpt.get();

            // تولید توکن بازیابی (۶ رقمی)
            String resetToken = String.format("%06d", random.nextInt(1000000));
            user.setResetToken(resetToken);
            user.setResetTokenExpiry(System.currentTimeMillis() + 15 * 60 * 1000); // 15 دقیقه

            userRepository.save(user);

            // TODO: اینجا سرویس ارسال ایمیل اضافه بشه
            logger.info("Reset token for {}: {}", email, resetToken);

            response.put("success", true);
            response.put("message", "کد بازیابی به ایمیل شما ارسال شد");
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            logger.error("Error in forgot password for email {}: {}", request.get("email"), e.getMessage(), e);
            response.put("success", false);
            response.put("message", "خطا در درخواست بازیابی رمز عبور");
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> resetPassword(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            if (email == null || token == null || newPassword == null) {
                response.put("success", false);
                response.put("message", "ایمیل، کد تأیید و رمز عبور جدید الزامی هستند");
                return ResponseEntity.badRequest().body(response);
            }
            ...
            // بررسی توکن و زمان انقضا
            if (user.getResetToken() == null || !user.getResetToken().equals(token)) {
                response.put("success", false);
                response.put("message", "کد تأیید نامعتبر است");
                return ResponseEntity.badRequest().body(response);
            }

            if (user.getResetTokenExpiry() == null || user.getResetTokenExpiry() < System.currentTimeMillis()) {
                response.put("success", false);
                response.put("message", "کد تأیید منقضی شده است");
                return ResponseEntity.badRequest().body(response);
            }
            ...
            logger.error("Error in reset password for email {}: {}", request.get("email"), e.getMessage(), e);
            response.put("success", false);
            response.put("message", "خطا در تغییر رمز عبور");
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
}