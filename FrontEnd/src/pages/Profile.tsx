...
const Profile: React.FC = () => {
  ...
  const TAB_CONFIG: TabConfig[] = useMemo(() => [
    { id: 'profile', label: '👤 اطلاعات پروفایل', icon: '👤', translationKey: 'profile.tabs.profile' },
    { id: 'payments', label: '💳 تاریخچه پرداخت‌ها', icon: '💳', translationKey: 'profile.tabs.payments' },
    { id: 'services', label: '🛎️ سرویس‌های من', icon: '🛎️', translationKey: 'profile.tabs.services' }
  ], []);
  ...
  const sanitizeUserData = useCallback((data: any): UserData => {
    return {
      ...
      mobile: typeof data.mobile === 'string' ? data.mobile.replace(/[^\d+]/g, '') : undefined,
      role: typeof data.role === 'string' ? data.role : undefined,
      profileImage: typeof data.profileImage === 'string' ? data.profileImage : undefined,
      firstName: typeof data.firstName === 'string' ? data.firstName.trim().slice(0, 50) : undefined,
      lastName: typeof data.lastName === 'string' ? data.lastName.trim().slice(0, 50) : undefined,
      nationalCode: typeof data.nationalCode === 'string' ? data.nationalCode.replace(/[^\d]/g, '').slice(0, 10) : undefined,
      passportNumber: typeof data.passportNumber === 'string' ? data.passportNumber.trim().slice(0, 20) : undefined,
      address: typeof data.address === 'string' ? data.address.trim().slice(0, 500) : undefined,
      userType: ['GUEST', 'VERIFIED', 'AMBASSADOR'].includes(data.userType) ? data.userType : 'GUEST'
    };
  }, []);
  ...
  const handleProfileUpdate = useCallback((updatedData: Partial<UserData>) => {
  ...
  }, [t, isValidImageUrl]); // اضافه کردن isValidImageUrl به dependencies
  ...
  const handleTabChange = useCallback((tabId: string) => {
    ...
    document.title = `${t('profile.tabs.' + tabId)} - ${t('profile.pageTitle')}`;
  }, [t]);

  // Helper functions
  const getUserTypeLabel = useCallback((userType: string = 'GUEST'): string => {
    return t(`profile.userTypes.${userType}.label`);
  }, [t]);

  const getUserTypeDescription = useCallback((userType: string = 'GUEST'): string => {
    ...
    >
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div role="tabpanel">
            {activeTab === 'profile' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="tab-profile">
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-3 space-x-reverse">
                      <div 
                        className={`w-3 h-3 rounded-full ${getUserTypeColor(user.userType)}`}
                        aria-hidden="true"
                      ></div>
