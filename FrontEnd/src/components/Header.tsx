...
import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react';import LoginModal from './LoginModal';
...
const Header: React.FC<HeaderProps> = () => {
  ...
  // زبان فعلی
  const currentLanguage = supportedLanguages[i18n.language as keyof typeof supportedLanguages] || supportedLanguages.fa;
  ...
  const profileMenuItems = useMemo(() => [
    ...
    { 
      ...
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192L5.636 18.364M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
    }
  ], [t]);
          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="hidden md:flex items-center space-x-4 space-x-reverse">
              ...
              <div className="relative">
                ...
                {/* منوی زبان */}
                {isLanguageMenuOpen && (
                  <div className={`absolute left-0 mt-2 w-32 rounded-lg shadow-lg border z-50 ${
                    theme === 'dark' 
              ...
              <button 
                ...
                className={`p-2 transition ${
                  theme === 'dark' ? 'text-yellow-400 hover:text-yellow-300' : 'text-gray-700 hover:text-blue-600'
                }`}
              >
                {theme === 'light' ? (
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isLoggedIn ? (
                <div className="relative">
                  ...
                  {/* منوی پروفایل */}
                  {isProfileMenuOpen && (
                    <div className={`absolute left-0 mt-2 w-64 rounded-2xl shadow-xl border z-50 ${
                      theme === 'dark' 
                        ? 'bg-gray-800 border-gray-700' 
                        : 'bg-white border-gray-200'
                    }`}>
                      ...
                      <div className="p-4 border-b border-gray-200 text-center">
                        <div className="flex flex-col items-center space-y-2">
                          ...
                          <div>
                            {t('header.yourPoints')}
                            <div className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>۱,۲۵۰</div>
                          </div>
                        </div>
                      </div>
