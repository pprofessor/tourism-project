import React, { useState, useEffect, useCallback } from "react";
import { sliderService, type Slide } from "../../services/sliderService";
import { sliderSettingsService } from "../../services/sliderSettingsService"; // این خط را اضافه کن
import {
  Box,
  Card,
  Button,
  IconButton,
  Dialog,
  DialogTitle,
  DialogContent,
  DialogActions,
  TextField,
  Alert,
  Snackbar,
  Chip,
  Typography,
  useTheme,
  useMediaQuery,
  CircularProgress,
  FormControlLabel,
  Switch,
} from "@mui/material";
import {
  Add,
  Edit,
  Delete,
  Visibility,
  VisibilityOff,
  Upload,
  NavigateBefore,
  NavigateNext,
  RadioButtonUnchecked,
  MoreHoriz,
  Settings,
} from "@mui/icons-material";

interface SliderFormData {
  title: string;
  description: string;
  buttonText: string;
  buttonLink: string;
  altText: string;
  seoTitle: string;
  seoDescription: string;
  transitionType: "fade" | "slide" | "zoom" | "flip";
  navigationType: "dots" | "arrows" | "dots_arrows" | "custom";
  customNavigation: string;
  slideInterval: number;
  transitionDuration: number;
  showTitle: boolean;
  sliderHeight: string;
  autoPlay: boolean;
}

const NavigationTypeSelector: React.FC<{
  value: string;
  onChange: (value: "dots" | "arrows" | "dots_arrows" | "custom") => void;
}> = ({ value, onChange }) => {
  const options = [
    {
      value: "dots",
      label: "نقطه‌ها",
      icon: <RadioButtonUnchecked sx={{ fontSize: 20 }} />,
      description: "نمایش نقاط برای ناوبری",
    },
    {
      value: "arrows",
      label: "فلش‌ها",
      icon: (
        <Box display="flex" gap={0.5}>
          <NavigateBefore sx={{ fontSize: 20 }} />
          <NavigateNext sx={{ fontSize: 20 }} />
        </Box>
      ),
      description: "نمایش فلش‌های چپ و راست",
    },
    {
      value: "dots_arrows",
      label: "ترکیبی",
      icon: (
        <Box display="flex" gap={0.5} alignItems="center">
          <NavigateBefore sx={{ fontSize: 16 }} />
          <RadioButtonUnchecked sx={{ fontSize: 16 }} />
          <NavigateNext sx={{ fontSize: 16 }} />
        </Box>
      ),
      description: "نمایش نقطه و فلش با هم",
    },
    {
      value: "custom",
      label: "سفارشی",
      icon: <MoreHoriz sx={{ fontSize: 20 }} />,
      description: "نمادهای سفارشی",
    },
  ];

  return (
    <Box sx={{ direction: "rtl" }}>
      <Typography variant="body2" color="text.secondary" gutterBottom>
        انتخاب نوع ناوبری:
      </Typography>
      <Box display="grid" gridTemplateColumns="1fr 1fr" gap={1}>
        {options.map((option) => (
          <Card
            key={option.value}
            sx={{
              p: 2,
              cursor: "pointer",
              border: value === option.value ? 2 : 1,
              borderColor: value === option.value ? "primary.main" : "divider",
              backgroundColor:
                value === option.value ? "action.selected" : "background.paper",
              borderRadius: 3,
              transition: "all 0.2s",
              "&:hover": {
                borderColor: "primary.light",
                backgroundColor: "action.hover",
              },
            }}
            onClick={() => onChange(option.value as any)}
          >
            <Box display="flex" alignItems="center" gap={1}>
              <Box
                sx={{
                  color:
                    value === option.value ? "primary.main" : "text.secondary",
                }}
              >
                {option.icon}
              </Box>
              <Box>
                <Typography variant="body2" fontWeight="medium">
                  {option.label}
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  {option.description}
                </Typography>
              </Box>
            </Box>
          </Card>
        ))}
      </Box>
    </Box>
  );
};

const SliderManagement: React.FC = () => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("md"));
  const [slides, setSlides] = useState<Slide[]>([]);
  const [loading, setLoading] = useState(true);
  const [openDialog, setOpenDialog] = useState(false);
  const [navigationSettingsOpen, setNavigationSettingsOpen] = useState(false);
  const [editingSlide, setEditingSlide] = useState<Slide | null>(null);
  const [imagePreview, setImagePreview] = useState<string>("");
  const [snackbar, setSnackbar] = useState({
    open: false,
    message: "",
    severity: "success" as "success" | "error",
  });
  const [uploadProgress, setUploadProgress] = useState(0);

  const [formData, setFormData] = useState<SliderFormData>({
    title: "",
    description: "",
    buttonText: "",
    buttonLink: "",
    altText: "",
    seoTitle: "",
    seoDescription: "",
    transitionType: "fade",
    navigationType: "dots_arrows",
    customNavigation: "default",
    slideInterval: 5000,
    transitionDuration: 500,
    showTitle: true,
    sliderHeight: "600px",
    autoPlay: true,
  });

  const loadSlides = useCallback(async () => {
    try {
      setLoading(true);
      const data = await sliderService.getAllSlides();
      setSlides(data);
    } catch (error) {
      setSnackbar({
        open: true,
        message: "خطا در بارگذاری اسلایدها",
        severity: "error",
      });
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    loadSlides();
  }, [loadSlides]);

  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    if (!file.type.startsWith("image/")) {
      setSnackbar({
        open: true,
        message: "فرمت تصویر نامعتبر است",
        severity: "error",
      });
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      setSnackbar({
        open: true,
        message: "سایز تصویر نباید بیشتر از 5MB باشد",
        severity: "error",
      });
      return;
    }

    const reader = new FileReader();
    reader.onload = (e) => setImagePreview(e.target?.result as string);
    reader.readAsDataURL(file);

    setUploadProgress(0);
    const interval = setInterval(() => {
      setUploadProgress((prev) => {
        if (prev >= 100) {
          clearInterval(interval);
          return 100;
        }
        return prev + 10;
      });
    }, 100);
  };

  const handleAddSlide = () => {
    setEditingSlide(null);
    setFormData({
      title: "",
      description: "",
      buttonText: "",
      buttonLink: "",
      altText: "",
      seoTitle: "",
      seoDescription: "",
      transitionType: "fade",
      navigationType: "dots_arrows",
      customNavigation: "default",
      slideInterval: 5000,
      transitionDuration: 500,
      showTitle: true,
      sliderHeight: "600px",
      autoPlay: true,
    });
    setImagePreview("");
    setOpenDialog(true);
  };

  const handleEditSlide = (slide: Slide) => {
    setEditingSlide(slide);
    setFormData({
      title: slide.title,
      description: slide.description,
      buttonText: slide.buttonText,
      buttonLink: slide.buttonLink,
      altText: slide.altText,
      seoTitle: slide.seoTitle || "",
      seoDescription: slide.seoDescription || "",
      transitionType: slide.transitionType || "fade",
      navigationType: slide.navigationType || "dots_arrows",
      customNavigation: slide.customNavigation || "default",
      slideInterval: slide.slideInterval || 5000,
      transitionDuration: slide.transitionDuration || 500,
      showTitle: slide.showTitle !== undefined ? slide.showTitle : true,
      sliderHeight: slide.sliderHeight || "600px",
      autoPlay: slide.autoPlay !== false,
    });
    setImagePreview(slide.image);
    setOpenDialog(true);
  };

  const handleSaveSlide = async () => {
    if (!formData.title.trim() || !imagePreview) {
      setSnackbar({
        open: true,
        message: "لطفاً عنوان اسلاید را وارد کنید",
        severity: "error",
      });
      return;
    }

    try {
      const slideData = {
        title: formData.title,
        description: formData.description,
        buttonText: formData.buttonText,
        buttonLink: formData.buttonLink,
        image: imagePreview,
        altText: formData.altText,
        seoTitle: formData.seoTitle,
        seoDescription: formData.seoDescription,
        isActive: true,
        displayOrder: editingSlide ? editingSlide.displayOrder : slides.length,
        transitionType: formData.transitionType,
        navigationType: formData.navigationType,
        customNavigation: formData.customNavigation,
        slideInterval: formData.slideInterval,
        transitionDuration: formData.transitionDuration,
        showTitle: formData.showTitle,
        sliderHeight: formData.sliderHeight,
        autoPlay: formData.autoPlay,
      };

      if (editingSlide) {
        await sliderService.updateSlide(editingSlide.id.toString(), slideData);
      } else {
        await sliderService.createSlide(slideData);
      }

      setOpenDialog(false);
      loadSlides();
      setSnackbar({
        open: true,
        message: editingSlide
          ? "اسلاید با موفقیت به‌روزرسانی شد"
          : "اسلاید جدید با موفقیت ایجاد شد",
        severity: "success",
      });
    } catch (error) {
      setSnackbar({
        open: true,
        message: "خطا در ذخیره اسلاید",
        severity: "error",
      });
    }
  };

  const handleDeleteSlide = async (id: number) => {
    if (!window.confirm("آیا از حذف این اسلاید مطمئن هستید؟")) return;

    try {
      await sliderService.deleteSlide(id.toString());
      setSnackbar({
        open: true,
        message: "اسلاید با موفقیت حذف شد",
        severity: "success",
      });
      loadSlides();
    } catch (error) {
      setSnackbar({
        open: true,
        message: "خطا در حذف اسلاید",
        severity: "error",
      });
    }
  };

  const handleToggleStatus = async (slide: Slide) => {
    try {
      await sliderService.toggleSlideStatus(
        slide.id.toString(),
        !slide.isActive
      );
      setSnackbar({
        open: true,
        message: slide.isActive ? "اسلاید غیرفعال شد" : "اسلاید فعال شد",
        severity: "success",
      });
      loadSlides();
    } catch (error) {
      setSnackbar({
        open: true,
        message: "خطا در تغییر وضعیت اسلاید",
        severity: "error",
      });
    }
  };

  if (loading) {
    return (
      <Box
        display="flex"
        justifyContent="center"
        alignItems="center"
        minHeight="400px"
        sx={{ direction: "rtl" }}
      >
        <CircularProgress />
      </Box>
    );
  }

  return (
    <Box sx={{ direction: "rtl" }}>
      <Box
        display="flex"
        justifyContent="space-between"
        alignItems="center"
        mb={3}
      >
        <Typography variant="h4" component="h1" fontWeight="bold">
          مدیریت اسلایدشو
        </Typography>
        <Box display="flex" gap={2}>
          <Button
            variant="outlined"
            startIcon={<Settings />}
            onClick={() => setNavigationSettingsOpen(true)}
            size={isMobile ? "medium" : "large"}
            sx={{ borderRadius: 3 }}
          >
            تنظیمات اسلایدشو
          </Button>
          <Button
            variant="contained"
            startIcon={<Add />}
            onClick={handleAddSlide}
            size={isMobile ? "medium" : "large"}
            sx={{ borderRadius: 3 }}
          >
            افزودن اسلاید
          </Button>
        </Box>
      </Box>
      <Box
        display="grid"
        gridTemplateColumns={{
          xs: "1fr",
          md: "1fr 1fr",
          lg: "1fr 1fr 1fr",
        }}
        gap={3}
      >
        {slides.map((slide) => (
          <Card
            key={slide.id}
            sx={{
              borderRadius: 3,
              overflow: "hidden",
              transition: "all 0.3s ease",
              "&:hover": {
                transform: "translateY(-4px)",
                boxShadow: theme.shadows[8],
              },
            }}
          >
            <Box position="relative">
              <Box
                sx={{
                  height: 200,
                  backgroundImage: `url(${slide.image})`,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                  position: "relative",
                }}
              >
                <Chip
                  label={slide.isActive ? "فعال" : "غیرفعال"}
                  color={slide.isActive ? "success" : "default"}
                  size="small"
                  sx={{
                    position: "absolute",
                    top: 8,
                    right: 8,
                    borderRadius: 2,
                  }}
                />
              </Box>

              <Box p={2}>
                <Typography variant="h6" noWrap title={slide.title}>
                  {slide.title}
                </Typography>
                <Typography
                  variant="body2"
                  color="text.secondary"
                  sx={{
                    display: "-webkit-box",
                    WebkitLineClamp: 2,
                    WebkitBoxOrient: "vertical",
                    overflow: "hidden",
                  }}
                >
                  {slide.description}
                </Typography>

                <Box display="flex" justifyContent="space-between" mt={2}>
                  <Box>
                    <IconButton
                      size="small"
                      onClick={() => handleToggleStatus(slide)}
                      title={slide.isActive ? "غیرفعال کردن" : "فعال کردن"}
                      sx={{ borderRadius: 2 }}
                    >
                      {slide.isActive ? <VisibilityOff /> : <Visibility />}
                    </IconButton>
                    <IconButton
                      size="small"
                      onClick={() => handleEditSlide(slide)}
                      title="ویرایش"
                      sx={{ borderRadius: 2 }}
                    >
                      <Edit />
                    </IconButton>
                  </Box>
                  <IconButton
                    size="small"
                    color="error"
                    onClick={() => handleDeleteSlide(slide.id)}
                    title="حذف"
                    sx={{ borderRadius: 2 }}
                  >
                    <Delete />
                  </IconButton>
                </Box>
              </Box>
            </Box>
          </Card>
        ))}
      </Box>
      {/* مدال افزودن/ویرایش اسلاید */}
      <Dialog
        open={openDialog}
        onClose={() => setOpenDialog(false)}
        maxWidth="md"
        fullWidth
        fullScreen={isMobile}
        sx={{ "& .MuiDialog-paper": { borderRadius: 3 } }}
      >
        <DialogTitle
          component="div"
          sx={{ textAlign: "right", borderBottom: 1, borderColor: "divider" }}
        >
          <Typography variant="h6" fontWeight="bold">
            {editingSlide ? "ویرایش اسلاید" : "افزودن اسلاید"}
          </Typography>
        </DialogTitle>

        <DialogContent sx={{ mt: 2 }}>
          <Box display="flex" flexDirection="column" gap={3}>
            {/* آپلود تصویر */}
            <Box sx={{ direction: "rtl" }}>
              <Typography variant="h6" gutterBottom>
                تصویر اسلاید
              </Typography>

              <Box
                sx={{
                  border: "2px dashed",
                  borderColor: "divider",
                  borderRadius: 3,
                  p: 3,
                  textAlign: "center",
                  cursor: "pointer",
                  position: "relative",
                  transition: "all 0.3s ease",
                  "&:hover": {
                    borderColor: "primary.main",
                    backgroundColor: "action.hover",
                  },
                }}
                onClick={() => document.getElementById("image-upload")?.click()}
              >
                <input
                  id="image-upload"
                  type="file"
                  accept="image/*"
                  onChange={handleImageUpload}
                  style={{ display: "none" }}
                />

                {imagePreview ? (
                  <Box>
                    <img
                      src={imagePreview}
                      alt="Preview"
                      style={{
                        maxWidth: "100%",
                        maxHeight: 200,
                        borderRadius: 12,
                      }}
                    />
                    {uploadProgress > 0 && uploadProgress < 100 && (
                      <Box mt={1}>
                        <CircularProgress
                          variant="determinate"
                          value={uploadProgress}
                        />
                        <Typography variant="body2" mt={1}>
                          {uploadProgress}% آپلود شده
                        </Typography>
                      </Box>
                    )}

                    <Box mt={2} display="flex" justifyContent="center">
                      <Button
                        variant="outlined"
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                        sx={{ borderRadius: 3 }}
                      >
                        ویرایش تصویر
                      </Button>
                    </Box>
                  </Box>
                ) : (
                  <Box>
                    <Upload
                      sx={{ fontSize: 48, color: "text.secondary", mb: 1 }}
                    />
                    <Typography variant="body1" color="text.secondary">
                      برای آپلود تصویر کلیک کنید
                    </Typography>
                    <Typography variant="body2" color="text.secondary" mt={1}>
                      فرمت‌های مجاز: JPG, PNG, GIF - حداکثر سایز: 5MB
                    </Typography>
                  </Box>
                )}
              </Box>
            </Box>

            {/* فیلدهای فرم */}
            <Box sx={{ direction: "rtl" }}>
              <Box display="flex" alignItems="center" gap={2} mb={2}>
                <TextField
                  fullWidth
                  label="عنوان اسلاید *"
                  value={formData.title}
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, title: e.target.value }))
                  }
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />
                <FormControlLabel
                  control={
                    <Switch
                      checked={formData.showTitle}
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          showTitle: e.target.checked,
                        }))
                      }
                      color="primary"
                    />
                  }
                  label="نمایش عنوان"
                  sx={{ textAlign: "right" }}
                />
              </Box>

              <TextField
                fullWidth
                label="توضیحات"
                value={formData.description}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    description: e.target.value,
                  }))
                }
                multiline
                rows={3}
                sx={{ mb: 2, borderRadius: 3 }}
                InputProps={{
                  sx: { textAlign: "right" },
                }}
                InputLabelProps={{
                  sx: { textAlign: "right" },
                }}
              />

              <Box display="flex" gap={2} mb={2}>
                <TextField
                  fullWidth
                  label="متن دکمه"
                  value={formData.buttonText}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      buttonText: e.target.value,
                    }))
                  }
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />
                <TextField
                  fullWidth
                  label="لینک دکمه"
                  value={formData.buttonLink}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      buttonLink: e.target.value,
                    }))
                  }
                  placeholder="#tours یا /tours"
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />
              </Box>

              <TextField
                fullWidth
                label="متن جایگزین تصویر"
                value={formData.altText}
                onChange={(e) =>
                  setFormData((prev) => ({ ...prev, altText: e.target.value }))
                }
                helperText="این متن برای دسترسی‌پذیری و سئو مهم است"
                sx={{ mb: 2, borderRadius: 3 }}
                InputProps={{
                  sx: { textAlign: "right" },
                }}
                InputLabelProps={{
                  sx: { textAlign: "right" },
                }}
              />
            </Box>

            {/* تنظیمات ناوبری */}
            <Box sx={{ direction: "rtl" }}>
              <Typography variant="h6" gutterBottom>
                تنظیمات ناوبری
              </Typography>
              <NavigationTypeSelector
                value={formData.navigationType}
                onChange={(value) =>
                  setFormData((prev) => ({ ...prev, navigationType: value }))
                }
              />
            </Box>

            {/* تنظیمات نمایش */}
            <Box sx={{ direction: "rtl" }}>
              <Typography variant="h6" gutterBottom>
                تنظیمات نمایش
              </Typography>

              <Box display="flex" gap={2} mb={2}>
                <TextField
                  fullWidth
                  label="ارتفاع اسلایدشو"
                  value={formData.sliderHeight}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      sliderHeight: e.target.value,
                    }))
                  }
                  placeholder="600px یا 70vh"
                  helperText="مثال: 500px, 600px, 70vh"
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />

                <TextField
                  fullWidth
                  label="زمان تعویض اسلاید (میلی‌ثانیه)"
                  type="number"
                  value={formData.slideInterval}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      slideInterval: parseInt(e.target.value) || 5000,
                    }))
                  }
                  helperText="زمان به میلی‌ثانیه (5000 = 5 ثانیه)"
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />
              </Box>

              <FormControlLabel
                control={
                  <Switch
                    checked={formData.autoPlay}
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        autoPlay: e.target.checked,
                      }))
                    }
                    color="primary"
                  />
                }
                label="پخش خودکار اسلایدها"
                sx={{ textAlign: "right" }}
              />
            </Box>
          </Box>
        </DialogContent>

        <DialogActions sx={{ justifyContent: "center", p: 3, gap: 2 }}>
          <Button
            onClick={() => setOpenDialog(false)}
            variant="outlined"
            sx={{ borderRadius: 3, minWidth: 120 }}
          >
            لغو
          </Button>
          <Button
            variant="contained"
            onClick={handleSaveSlide}
            disabled={!formData.title || !imagePreview}
            sx={{ borderRadius: 3, minWidth: 120 }}
          >
            {editingSlide ? "بروزرسانی" : "ذخیره"}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog
        open={navigationSettingsOpen}
        onClose={() => setNavigationSettingsOpen(false)}
        maxWidth="md"
        fullWidth
      >
        <DialogTitle component="div" sx={{ textAlign: "right" }}>
          <Typography variant="h6" fontWeight="bold">
            تنظیمات اسلایدشو
          </Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ direction: "rtl", mt: 2 }}>
            {/* تنظیمات ناوبری */}
            <Box sx={{ mb: 4 }}>
              <Typography variant="h6" gutterBottom>
                تنظیمات ناوبری
              </Typography>
              <NavigationTypeSelector
                value={formData.navigationType}
                onChange={(value) =>
                  setFormData((prev) => ({ ...prev, navigationType: value }))
                }
              />
            </Box>

            {/* تنظیمات نمایش */}
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" gutterBottom>
                تنظیمات نمایش
              </Typography>

              <Box display="flex" gap={2} mb={2}>
                <TextField
                  fullWidth
                  label="ارتفاع اسلایدشو"
                  value={formData.sliderHeight}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      sliderHeight: e.target.value,
                    }))
                  }
                  placeholder="600px یا 70vh"
                  helperText="مثال: 500px, 600px, 70vh"
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />

                <TextField
                  fullWidth
                  label="زمان تعویض اسلاید (میلی‌ثانیه)"
                  type="number"
                  value={formData.slideInterval}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      slideInterval: parseInt(e.target.value) || 5000,
                    }))
                  }
                  helperText="زمان به میلی‌ثانیه (5000 = 5 ثانیه)"
                  sx={{ borderRadius: 3 }}
                  InputProps={{
                    sx: { textAlign: "right" },
                  }}
                  InputLabelProps={{
                    sx: { textAlign: "right" },
                  }}
                />
              </Box>

              <FormControlLabel
                control={
                  <Switch
                    checked={formData.autoPlay}
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        autoPlay: e.target.checked,
                      }))
                    }
                    color="primary"
                  />
                }
                label="پخش خودکار اسلایدها"
                sx={{ textAlign: "right" }}
              />
            </Box>
          </Box>
        </DialogContent>
        <DialogActions sx={{ justifyContent: "center", p: 3, gap: 2 }}>
          <Button
            onClick={() => setNavigationSettingsOpen(false)}
            variant="outlined"
            sx={{ borderRadius: 3, minWidth: 120 }}
          >
            لغو
          </Button>
          <Button
            variant="contained"
            onClick={async () => {
              try {
                // ذخیره تنظیمات فعلی
                await sliderSettingsService.saveSettings({
                  sliderHeight: formData.sliderHeight,
                  autoPlay: formData.autoPlay,
                  slideInterval: formData.slideInterval,
                  navigationType: formData.navigationType,
                  transitionType: formData.transitionType,
                });
                setNavigationSettingsOpen(false);
                // نمایش پیام موفقیت
                setSnackbar({
                  open: true,
                  message: "تنظیمات با موفقیت ذخیره شد",
                  severity: "success",
                });
              } catch (error) {
                setSnackbar({
                  open: true,
                  message: "خطا در ذخیره تنظیمات",
                  severity: "error",
                });
              }
            }}
            sx={{ borderRadius: 3, minWidth: 120 }}
          >
            ذخیره
          </Button>
        </DialogActions>
      </Dialog>
      <Snackbar
        open={snackbar.open}
        autoHideDuration={6000}
        onClose={() => setSnackbar((prev) => ({ ...prev, open: false }))}
        anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
      >
        <Alert
          severity={snackbar.severity}
          onClose={() => setSnackbar((prev) => ({ ...prev, open: false }))}
          sx={{ borderRadius: 3 }}
        >
          {snackbar.message}
        </Alert>
      </Snackbar>
    </Box>
  );
};

export default SliderManagement;
